...
module App =
    ...
    let renderTree renderer view handler model =
        view model
        |> renderer.Render handler

    let draw renderState renderer post state= 
        let tree = renderTree renderer state.AppState.View post state.AppState.Model
        let (Some currentTree) = state.CurrentTree
        let (Some rootNode) = state.Node 
        let patches = renderer.Diff currentTree tree
        renderer.Patch rootNode patches |> ignore
        state.JsCalls |> List.iter (fun i -> i())
        { state with 
            CurrentTree = Some tree
            RenderState = renderState
            JsCalls = [] }

    let scheduleDraw post (scheduler:MailboxProcessor<ScheduleMessage>) currentState =
        match currentState with
        | NoRequest ->
            window.requestAnimationFrame(fun _ -> Draw |> post) 
//            scheduler.Post(PingIn(1000./60., (fun() -> Draw |> post)))
            InProgress
        | InProgress -> InProgress

    let start<'TModel,'TMessage> (renderer:Renderer<'TMessage>) (app:App<'TModel,'TMessage>) =
        let startElem =
        ...
        MailboxProcessor.Start(fun inbox ->
            let post (message:AppMessage<'TMessage>) =
                inbox.Post (message)
            ...
            let rec loop state =
                async {
                    ...
                    | None,_ ->
                        let tree = renderTree renderer state.AppState.View postMessage state.AppState.Model
                    ...
                    | Some rootNode, Some currentTree ->
                        ...
                        | Message msg ->
                            ...
                            let renderState = scheduleDraw post scheduler state.RenderState
                            let state' = 
                                {state with 
...
//                                    InProgress
//                                | InProgress -> InProgress
//                            let state' = draw state.AppState.View post rootNode model' jsCalls
                            return! loop state'//state.JsCalls @ jsCalls }
                        | Draw -> 
                            match state.RenderState with
                            | InProgress ->
                                DrawStarted |> notifySubscribers state.Subscribers

                                let state' = draw NoRequest renderer postMessage state
